...
const homeStyles = `
  /* Simulación de Smooth Scroll nativo mejorado */
  html {
    scroll-behavior: smooth;
  }

  /* Animación de Flotación (Levitación) */
  @keyframes float {
  ...
  /* Gradiente animado de fondo sutil */
  ...
  /* Intro Animations (Cinematic Reveal) */
...
// --- HOOK DE REVELADO AL SCROLL (GSAP STYLE) ---
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        // Solo se activa una vez al entrar
...
const Reveal = ({ children, delay = 0, className = "" }) => {
  ...
  // Usamos una curva Bezier personalizada para imitar GSAP 'power3.out'
...
const IntroOverlay = ({ onComplete }) => {
  ...
  useEffect(() => {
    // Solo manejamos el fade out final porque la entrada la hace CSS nativo
    const t1 = setTimeout(() => setStage(1), 2800); // Inicia fade out de pantalla
    const t2 = setTimeout(() => onComplete(), 3600); // Quita del DOM
  ...
  return (
    <div className={`fixed inset-0 z-[9999] bg-white flex flex-col items-center justify-center transition-all duration-1000 ease-in-out ${stage >= 1 ? 'opacity-0 scale-105 pointer-events-none' : 'opacity-100 scale-100'}`}>
      ...
      <div className="relative flex flex-col items-center justify-center">
        ...
        {/* Logo más grande y estilizado con latido cinemático */}
        ...
        {/* Tipografía elegante letra por letra */}
      ...
      <div className="relative overflow-hidden bg-white">
        ...
        <div className="lg:absolute lg:inset-y-0 lg:right-0 lg:w-1/2 lg:h-full flex items-center justify-center pointer-events-none">
             <div className="relative w-full h-64 sm:h-72 md:h-96 lg:h-full animate-float">
                 {/* Máscara de degradado para integrar la imagen */}
